					...
					<c:choose>
						...
						<div class="col-md-5">
							...
							<div class="lecB-border">
								...
								<p class="p-ml">강의인원:<span class="span-ml">${lecture.enrollmentCount}명</span></p>
								<div class="d-flex">
									<div class="col-sm-5">
										<span>학점:</span>
...
<script>
	...
	$(document).ready(function(){
		...
		<c:forEach items="${currentLec}" var="lec" varStatus="lecCnt">
			var prevTime = "";
			var title = '${lec.lecLectTitle}';
			<c:forEach items="${lec.lectureScheduleList}" var="time" varStatus="timeCnt">
				var schedule = ${time.scheCd};
				var scheduleStr = '${time.scheCd}'; 
	 			if(${timeCnt.index}==0){
	 				$(schedule).text(title);
	 				$(schedule).css("background-color", blackgroundColor[${lecCnt.index}]);
	 			}else if(${timeCnt.index}>0 && (prevTime[0] === scheduleStr[0])){
	 				$("#"+prevTime).removeClass("inner-style").addClass("remove-bottom");
	 				$(schedule).css("background-color", blackgroundColor[${lecCnt.index}]);
	 			}else{
	 				$(schedule).text(title);
	 				$(schedule).css("background-color", blackgroundColor[${lecCnt.index}]);
	 			}
	 			prevTime = '${time.scheCd}';
			</c:forEach>
		</c:forEach>
