	...
	<div class="h-100">
		<div class="custom-card">
			...
			<div class="gradInfo-body">
				...
				|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;이수구분,학년,학기,강의 상태를 선택하고 검색버튼을 눌러주세요.
			</div>
			<div class="gradInfo-body">
				<span class="span-title">강의등록,수정,시간등록</span>
				|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;강의 요청, 수정, 시간 등록 시에 강의 상태는 변경이 진행되지 않습니다. 강의 상태 변경을 원할 시 담당 교직원에게 문의하세요.
			</div>
			<div class="gradInfo-body" id="gradeStudents"></div>
			<div style="display: flex; justify-content: flex-end; width: 100%; margin-top: -50px;">
				<button id="insertRow" class="btn btn-outline-info mb-3"  style="padding: 12px; border-radius: 10px; type="button" data-bs-toggle="modal" data-bs-target="#staticBackdrop">강의요청</button>
...
$(staticBackdrop).on('show.bs.modal',function(event){
	...
	let prCd = ${detail.prCd};
	let setting = {
		url : `${pageContext.request.contextPath}/lecture/lectureRequest.do`,
		data : {
			prCd : prCd
		},
		success : function(resp) {
...
$(staticBackdrop2).on('show.bs.modal',function(event){
	...
	if($checkLecCd.length==1){
        ...
        if(lecCd){
           let setting = {
             url : `${pageContext.request.contextPath}/lecture/lectureFix.do`,
             dataType : "html",
             data : {
            	 prCd : prCd,
            	 lecCd : lecCd
...
$(staticBackdrop3).on('show.bs.modal',function(event){
    ...
    if($checkLecCd.length==1){
        ...
        if(lecCd){
           let setting = {
             url : `${pageContext.request.contextPath}/lectureSchedule/lectureScheduleInsert2.do`,
             dataType : "html",
             data : {
            	 lecCd:lecCd
             },
