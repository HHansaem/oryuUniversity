...
public class PaginationInfo<T> {
	...
	String aPtrn2 = "<a class='page-link' href=''javascript:fn_paging2({0});'' data-page=''{0}''>{1}</a>";
	...
	public String getPagingHTML2() {
		...
		html.append(MessageFormat.format(liPtrn, startPage<=1 ? "disabled" : ""));
		html.append(
			MessageFormat.format(aPtrn2, startPage-1, "이전")	
		);
		html.append("</li>");
		...
		for(long page = startPage; page <= endPage; page++ ) {
			if(page==currentPage) {
				...
				html.append(MessageFormat.format(liPtrn, ""));
				html.append(
					MessageFormat.format(aPtrn2, page, page)	
				);
				html.append("</li>");
			}
		}
		...
		html.append(MessageFormat.format(liPtrn, endPage >= totalPage ? "disabled" : ""));
		html.append(
			MessageFormat.format(aPtrn2, endPage+1, "다음")	
		);
		html.append("</li>");
